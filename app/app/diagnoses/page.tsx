...
export default async function DiagnosesPage() {
  ...
  return (
    <div className="mx-auto w-full max-w-3xl">
      ...
      <h1 className="mt-1 text-2xl font-semibold tracking-tight">Your diagnoses</h1>
      <p className="mt-1 text-sm text-black/50">Every piece of work you’ve submitted, and where each one stands.</p>

      {rows.length === 0 ? (
        <div className="mt-8 rounded-2xl border border-black/10 p-6 text-sm text-black/60">
          No diagnoses yet.{' '}
          <Link href="/diagnosis" className="underline underline-offset-4">
            Start one
          </Link>
          .
        </div>
        ...
        <ul className="mt-6 flex flex-col gap-3">
          {rows.map((r) => (
            <li key={r.id} className="rounded-2xl border border-black/10 p-5">
              <div className="flex items-start justify-between gap-4">
                <div className="min-w-0">
                  <div className="flex flex-wrap items-center gap-2">
                    ...
                    <span className="text-xs text-black/40">{fmtDate(r.createdAt)}</span>
                  </div>
                  <div className="mt-2 text-sm font-medium text-black/80">{r.constraintName ?? 'Read in progress'}</div>
                ...
                <div className="shrink-0">
                  {VIEWABLE.has(r.status) ? (
                    <Link href={`/results/${r.token}`} className="text-xs underline underline-offset-4">
                      View result
                    </Link>
                  ) : (
                    <span className="text-xs text-black/40">We’ll email you</span>
                  )}
                </div>
              </div>
            </li>
