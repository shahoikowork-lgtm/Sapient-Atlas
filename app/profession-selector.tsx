  ...
  return (
    <div>
            ...
            <button
              ...
              className={`rounded-full px-3.5 py-1.5 text-label transition-colors ${
                on
                  ...
                  : 'border border-hairline bg-surface text-text-secondary hover:border-hairline-strong hover:text-ink'
              }`}
            >
              {f.key}
            </button>
      ...
      <div className="mt-5 rounded-2xl border border-hairline bg-surface p-5">
        <p className="text-body-lg text-ink">{field.line}</p>
        <p className="mt-3 text-label text-muted">
          {field.open ? (
            <span className="text-grow">Sprint open today · $149, once, no subscription.</span>
          ) : (
